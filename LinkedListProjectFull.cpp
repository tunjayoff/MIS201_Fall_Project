...
void Color(int color);                          //For colors
void push(node** head_ref, string url);         //Push data to the beginning
void append(node** head_ref, string url);       //Add the data at the end
void deleteNode(node** head_ref, node* del);    //Delete one node
void deleteList(node** head_ref);               //Delete whole list
void insertAfter(node* prev_node, string url);  //Insert a node after a given node
void history(node* head);                       //Show whole nodes in the linked list
node* search(node* head, string website_name);  //Search for a data and return the node which contain the data

//A new node outside of driver code, to contain the current node
